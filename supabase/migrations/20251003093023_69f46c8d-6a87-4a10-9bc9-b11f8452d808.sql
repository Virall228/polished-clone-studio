...
CREATE POLICY "Captains update own teams"
ON public.teams
AS PERMISSIVE
FOR UPDATE
TO authenticated
...
DROP POLICY IF EXISTS "Users can manage their tournament participation" ON public.tournament_participants;
...
WITH CHECK (
  ...
  OR team_id IN (
    ...
    WHERE t.captain_id = (SELECT public.current_user_id())
  )
);
...
DROP POLICY IF EXISTS "News are viewable (author or published)" ON public.news;

CREATE POLICY "News viewable if published or own"
ON public.news
AS PERMISSIVE
FOR SELECT
TO public
...
CREATE POLICY "Authors manage own news"
ON public.news
AS PERMISSIVE
FOR ALL
TO authenticated
...
DROP POLICY IF EXISTS "Users manage their own terms agreements" ON public.terms_agreements;
...
DROP POLICY IF EXISTS "Users can view their roles or admins view all" ON public.user_roles;

CREATE POLICY "Users view own roles or admins all"
...
DROP POLICY IF EXISTS "Team invites viewable by members and public if active" ON public.team_invites;

CREATE POLICY "Team invites viewable if active or member"
...
CREATE POLICY "Captains manage team invites"
ON public.team_invites
AS PERMISSIVE
FOR ALL
TO authenticated
...
CREATE POLICY "Captains manage tournament invites"
ON public.tournament_invites
AS PERMISSIVE
FOR ALL
TO authenticated
